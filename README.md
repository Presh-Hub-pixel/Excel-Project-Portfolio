
---

# 📊 Excel Project Portfolio

Welcome to my Excel Project Repository!

This portfolio showcases my expertise in **data analysis, dashboard development, and business intelligence reporting using Microsoft Excel**. Each project demonstrates my ability to transform raw data into meaningful insights that support strategic business decisions.

As a Data Analyst, I focus on combining analytical thinking with data visualization techniques to deliver clear, actionable, and business-ready dashboards.

---

## 📌 Table of Contents 🗂️

1️⃣ 👩‍💻 About Me

2️⃣ 📊 Project Overview – Amazon Sales Dashboard

3️⃣ 📊 Project Overview – US Superstore Analysis Dashboard

4️⃣ 🛠 Technologies & Tools

5️⃣ 🔐 Excel Best Practices Applied

6️⃣ 📩 Contact Information

---

## 👩‍💻 About Me

Hello! I'm **Rotimi Precious Eniola**, a Data Analyst | Excel Data Specialist | Business Intelligence Enthusiast.

I am passionate about transforming raw datasets into actionable insights using Excel and analytical tools. My work focuses on data cleaning, visualization, dashboard development, and business intelligence reporting that supports data-driven decision-making.

🔹 **Professional Strengths**
✔ Data Cleaning & Transformation

✔ Dashboard Development & KPI Reporting

✔ Business Intelligence & Data Storytelling

✔ Advanced Excel Functions & Automation

✔ Data Visualization for Decision-Making

---

## 📊 Project Overview – Amazon Sales Dashboard 🛒

📌 **Project Description**

This project presents a comprehensive analysis of Amazon sales performance using Microsoft Excel. The dashboard provides insights into:

* Revenue performance by product category
* Regional sales distribution
* Monthly and seasonal sales trends
* Top-performing products and segments
* Key performance indicators (KPIs) for business monitoring

The objective is to convert raw transactional data into an interactive, executive-ready dashboard that supports strategic decision-making.

🔗 **View Dashboard:** [Amazon Sales Dashboard](INSERT_AMAZON_DASHBOARD_LINK_HERE)

🔍 **Key Features**

* Dynamic Pivot Tables & Pivot Charts
* Interactive slicers and filters
* Revenue and performance tracking
* Month-on-Month trend analysis
* Power Query for data transformation
* Clean and professional dashboard layout

🎯 **Business Impact**

* Identifies top-performing product categories
* Highlights seasonal revenue trends
* Tracks key business KPIs in real time
* Supports pricing and marketing strategy decisions

---

## 📊 Project Overview – US Superstore Analysis Dashboard 📦

📌 **Project Description**

This project analyzes the **US Superstore dataset** using Microsoft Excel to uncover key business insights around sales, profit, and customer behavior. The dashboard highlights performance across regions, product categories, and shipping segments.

🔗 **View Dashboard:** [US Superstore Dashboard](INSERT_US_SUPERSTORE_DASHBOARD_LINK_HERE)

🔍 **Key Features**

* Sales and profit analysis by region
* Category and sub-category performance breakdown
* Customer segment analysis
* Shipping mode performance evaluation
* Interactive filters and slicers
* Time-based trend analysis

🎯 **Business Impact**

* Identifies most profitable and loss-making segments
* Supports regional performance optimization
* Improves inventory and supply chain decisions
* Enhances strategic business planning

---

## 🛠 Technologies & Tools

📊 Microsoft Excel
📂 Power Query
📈 Pivot Tables & Pivot Charts
🧮 Advanced Excel Functions
📊 Data Visualization & KPI Reporting

---

## 🔐 Excel Best Practices Applied

✔ Structured Excel tables for scalable analysis
✔ Power Query for efficient data cleaning and transformation
✔ Optimized formulas for performance improvement
✔ Clear KPI definitions and dashboard structuring
✔ Professional formatting and visual consistency

---

## 📩 Contact Information

**Rotimi Precious Eniola**
Data Analyst | Excel Data Specialist | Business Intelligence Enthusiast

📧 Email: [rotimipreciousenny04@gmail.com](mailto:rotimipreciousenny04@gmail.com)
🔗 LinkedIn: [Rotimi Precious Eniola](https://www.linkedin.com/in/precious-rotimi-846281396?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app)

---

⭐ *Transforming raw data into meaningful business intelligence through Excel analytics.*

---

